...
public final class OneDimAveragingPhaser {
     ...
     * @param tasks The number of threads/tasks to use to compute the solution
     */
    public static void runParallelFuzzyBarrier(int iterations, double[] myNew, double[] myVal, int n, int tasks) {
        ...
        for (int ii = 0; ii < tasks; ++ii) {
            final int finalII = ii; // declare ii as final
            ...
            threads[ii] = new Thread(() -> {
                ...
                for (int iter = 0; iter < iterations; ++iter) {
                    ...
                    int left = finalII * chunkSize + 1;
                    int right = left + chunkSize - 1;
                    if (right > n) {
                        right = n;
                    }

                    for (int currentPhase = left; currentPhase <= right; ++currentPhase) {
                        threadPrivateMyNew[currentPhase] = (threadPrivateMyVal[currentPhase - 1] + threadPrivateMyVal[currentPhase + 1]) / 2.0;
                    }

                    int currentPhase = phs[finalII].arrive();
                    if (finalII - 1 >= 0) {
                        phs[finalII - 1].awaitAdvance(currentPhase);
                    }
